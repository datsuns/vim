...
#ifdef AMIGA
#define DFLT_EFM                                                               \
  "%f>%l:%c:%t:%n:%m,%f:%l: %t%*\\D%n: %m,%f %l %t%*\\D%n: "                   \
  "%m,%*[^\"]\"%f\"%*\\D%l: %m,%f:%l:%m,%f|%l| %m"
#else
#if defined(MSWIN)
#define DFLT_EFM                                                               \
  "%f(%l): %t%*\\D%n: %m,%f(%l\\,%c): %t%*\\D%n: %m,%f(%l) \\=: %t%*\\D%n: "   \
  "%m,%*[^\"]\"%f\"%*\\D%l: %m,%f(%l) \\=: %m,%*[^ ] %f %l: "                  \
...
#else // Unix, probably
#define DFLT_EFM                                                               \
  "%*[^\"]\"%f\"%*\\D%l: %m,\"%f\"%*\\D%l: %m,%-Gg%\\?make[%*\\d]: *** "       \
  "[%f:%l:%m,%-Gg%\\?make: *** [%f:%l:%m,%-G%f:%l: (Each undeclared "          \
...
#define SHM_LEN                                                                \
  30 // max length of all flags together
...
#define GO_NOCAPTION 'z' // remove caption bar
#define GO_ALL "!aAbcdefFghilLmMpPrRtTvkz"
